...
class StorageManager:
    ...
    def purge_database():
        ...
        for collection_name in db.list_collection_names():
            warn(f"\tDropping collection {collection_name} (collection in {db.name}))")
            db.drop_collection(collection_name)
        client.close()

    def setup_collections(self, utterance, conversation, speaker, convokitmeta):
        """
    ...
    def __eq__(self, other):
        if not isinstance(other, StorageManager):
            ...
            return self.storage_type == other.storage_type and self.full_name == other.full_name
