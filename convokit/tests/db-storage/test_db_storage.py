...
class DBStorage(unittest.TestCase):
    ...
    def test_db_corpusComponent(self):
        ...
        storage.setup_collections(Utterance, Conversation, Speaker, ConvoKitMeta)
        ...
        storage_ = StorageManager(storage_type="db", corpus_id="direct_storage", in_place=True)
        storage_.setup_collections(Utterance, Conversation, Speaker, ConvoKitMeta)

        # Test persistent storage
        self.assertEqual(storage_.speakers["Bob"].utterances["0"], storage_.utterances["0"])
        self.assertEqual(storage_.utterances["1"].speaker, storage_.speakers["Jim"])

        self.assertEqual(storage_.speakers["Bob"].utterances["0"].text, BOBS_TEXT)
        self.assertEqual(storage_.utterances["1"].speaker.id, "Jim")
        self.assertEqual(storage_.conversations[None].speaker_ids, ["Bob", "Jim"])

        self.assertEqual(storage_.conversations[None].get_utterance_ids(), ["0", "1"])
    ...
    def test_db_migration(self):
        _storage = StorageManager(storage_type="db", corpus_id="not_the_default_corpus")
        _storage.purge_all_collections()

        del _storage

        outside_storage = StorageManager(storage_type="db", corpus_id="not_the_default_corpus")
        outside_storage.setup_collections(Utterance, Conversation, Speaker, ConvoKitMeta)
        ...
        self.assertEqual(outside_storage.metas["utterance_0"], {"text-len": text_len})

        self.assertEqual(corpus.get_utterance("0").meta, {"text-len": text_len})
        self.assertEqual(corpus.get_utterance("0").speaker.meta, {"height": height})
        self.assertEqual(corpus.get_utterance("0").retrieve_meta("text-len"), text_len)
        self.assertEqual(corpus.get_speaker("Bob").retrieve_meta("height"), height)
    ...
    def test_init_corpus_from_db(self):
        ...
        u0 = Utterance(id="0", text="hello world!!!", speaker=s0, storage=storage)
        self.assertEqual(u0.fields["speaker_id"], "alice")
        self.assertEqual(u0.speaker.id, "alice")

        u1 = Utterance(id="1", text="my name is bob", speaker=s1, storage=storage)
        u2 = Utterance(id="2", text="this is a test", speaker=s2, storage=storage)

        corpus = Corpus(utterances=[u0, u1, u2], storage_type="db", corpus_id="testo")
