...
KeyText = "text"
DefinedKeys = {
    KeyId, KeySpeaker, KeyConvoId, KeyReplyTo, KeyTimestamp, KeyText
}
KeyMeta = "meta"
KeyVectors = "vectors"


def load_uttinfo_from_dir(dirname, utterance_start_index, utterance_end_index,
...
def load_speakers_data_from_dir(filename, exclude_speaker_meta):
    speaker_file = "speakers.json" if "speakers.json" in os.listdir(
        filename) else "users.json"
...
def unpack_binary_data_for_utts(utterances, filename, utterance_index,
                                exclude_meta, KeyMeta):
    """
...
def unpack_binary_data(filename, objs_data, object_index, obj_type,
                       exclude_meta):
    """
        ...
        if field_types[0] == "bin" and field not in exclude_meta:
            with open(
                    os.path.join(filename,
                                 field + "-{}-bin.p".format(obj_type)),
                    "rb") as f:
                l_bin = pickle.load(f)
            for obj, data in objs_data.items():
                metadata = data['meta'] if len(
                    data) == 2 and 'vectors' in data else data
...
def dump_helper_bin(d: ConvoKitMeta,
    ...
    for k, v in d.items():
        ...
        try:
            if obj_idx[k][0] == "bin":
                d_out[k] = "{}{}{}".format(BIN_DELIM_L, len(d_bin[k]),
                                           BIN_DELIM_R)
                d_bin[k].append(v)
            else:
                d_out[k] = v
    ...
    with open(os.path.join(dir_name, filename), "w") as f:
        ...
        for obj_id in corpus.get_object_ids(obj_type):
            objs[obj_id][KeyMeta] = dump_helper_bin(
                corpus.get_object(obj_type, obj_id).meta, d_bin,
                fields_to_skip.get(obj_type, []))
...
def add_conv_meta_df(conversations_df, corpus):
    ...
    conversations_df.columns = [
        col.replace('meta.', '') for col in conversations_df.columns
    ]
