...
from convokit import StorageManager
...
class TestQuestionSentences(unittest.TestCase):
    def test_use_caps(self):
        ...
        transformer = QuestionSentences(input_field='sentences',
        ...
        for utterance, expected_sentences in zip(
            ...
            self.assertListEqual(expected_sentences,
                                 utterance.get_info('questions'))
    ...
    def test_dont_use_caps(self):
        ...
        expected_sentences_list = [['Are you Aaron Burr, sir?'],
                                   ["Who's asking?"]]
        for utterance, expected_sentences in zip(
                transformed_corpus.iter_utterances(), expected_sentences_list):
            self.assertListEqual(expected_sentences,
