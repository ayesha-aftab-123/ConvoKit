...
class PromptTypes(Transformer):
    ...
    def transform(
        ...
        corpus.set_vector_matrix(
            self.output_field + "__reference_repr",
        ...
        for id, assign, dist in zip(prompt_df.index, prompt_assigns, prompt_min_dists):
            ...
            corpus.get_utterance(id).add_meta(
                self.output_field + "__prompt_type_dist.%s" % self.default_n_types,
                float(dist),
            )
        for id, assign, dist in zip(reference_df.index, reference_assigns, reference_min_dists):
            ...
            corpus.get_utterance(id).add_meta(
                self.output_field + "__reference_type_dist.%s" % self.default_n_types,
                float(dist),
            )
        return corpus
    ...
    def _transform_utterance_side(self, utterance, side):
        ...
        utterance.add_meta(
            self.output_field + "__%s_type_dist.%s" % (side, self.default_n_types),
            float(min_dist),
        )
        utterance.add_meta(
        ...
        for key in to_load:
            ...
            for k in ["prompt_df", "reference_df"]:
                self.type_models[key][k] = pd.read_csv(
                    os.path.join(model_dir, "%s.%s.tsv" % (k, key)),
                    sep="\t",
                    index_col=0,
                )
            ...
            if load_train_corpus:
                ...
                for k in ["prompt_df", "reference_df"]:
                    self.train_types[key][k] = pd.read_csv(
                        os.path.join(model_dir, "train_%s.%s.tsv" % (k, key)),
                        sep="\t",
                        index_col=0,
                    )
